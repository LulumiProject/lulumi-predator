...
describe('BrowserMainView.vue', () => {
  before(async () => {
    ...
    vm.onNewTab(undefined, 'https://github.com/LulumiProject/lulumi-predator', true);
    vm.onTabClose(0);
    await vm.$nextTick();
  });
  ...
  describe('functions', () => {
    ...
    describe('methods.getTabObject()', () => {
      it('can call navigateTo method from certain tab instance to let the webview navigate to somewhere', () => {
        vm.getTab().navigateTo('https://github.com/LulumiProject/lulumi-predator');
        expect(vm.getTabObject().url).to.equal('https://github.com/LulumiProject/lulumi-predator');
      });
    });
